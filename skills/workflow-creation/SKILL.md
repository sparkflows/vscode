...
description: Generates import-ready Sparkflows workflow JSON from a natural language description. A workflow is a Spark data processing flow — use this skill when a user wants to read, transform, filter, join, aggregate, clean, validate, visualize, or write data using Spark nodes (e.g. "read a CSV and filter rows", "join two tables and save to Hive", "train an ML model"). Do NOT use this skill when the user asks to run, sequence, or orchestrate multiple workflows — that is pipeline-creation. Only use this skill for creating new workflows, not modifying existing ones.
---
...
A Sparkflows workflow JSON has a `nodes` array and an `edges` array at its root. Each entry in `workflow-nodes` is the **complete, unmodified JSON content** of a node file — loaded verbatim from the [workflow-nodes](../../workflow-nodes/) directory and inserted as-is. Edges are then defined separately to connect those nodes by their assigned IDs. The workflow schema in [workflow-schema](../../workflow-references/workflow-schema.md) defines all root-level keys and the exact edge format.
...
Read [workflow-node-index](../../workflow-references/workflow-node-index.md) in full. This is a short file (~1–2 pages) that describes each node category in 2–3 lines. Use it to determine which catalog files are relevant to the user's request.
...
Based on the index, load only the catalog files from [workflow-nodes-catalogs](../../workflow-references/workflow-nodes-catalogs/) that cover the operations in the user's request. Use the catalog entries to match each operation to the best available Sparkflows node.
...
For each identified node, load its `.json` file from the [workflow-nodes](../../workflow-nodes/) directory using the path from the catalog. Read the full file contents to check engine compatibility before injecting into the workflow.
...
Read [workflow-schema](../../workflow-references/workflow-schema.md) to confirm:
...
Open one file from [example-workflows](../../workflow-references/example-workflows/) to verify how a complete workflow looks when assembled — particularly how node JSON objects sit inside the `nodes` array and how edges reference them.
...
- The `edges` array is built from your DAG using the format defined in [workflow-schema](../../workflow-references/workflow-schema.md)
